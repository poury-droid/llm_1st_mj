...
numsacc = [1, 2, 3, 4, 5];
const sum4 = numsacc.reduce((acc, n) => acc + n, 0);
console.log(sum4);
...
console.log(["안", "녕", "하", "세", "요"].reduce((acc, s) => acc + s, ""));
// ── 섹션 4: 객체 배열의 합계 ──
...
console.log(scores.reduce((acc, n) => (acc > n ? n : acc)));
...
const countsa = alpha.reduce((acc, a) => {
  acc[a] = (acc[a] ?? 0) + 1;
  return acc; // 매 바퀴 acc 를 돌려줘야 합니다
}, {});
